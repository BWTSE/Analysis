...
own_quality0.with <- extendable_model(
  ...
  base_formula = "quality_post_task ~ 1  + (1 | session) + sonarqube_issues.s + var_names_new_good.ratio + reused_logic_constructor + reused_logic_validation + high_debt_version",
    base_priors = c(
...
own_quality_post <- posterior_predict(own_quality23.all, newdata = data.frame(high_debt_version = c("false", "true"), session = NA, sonarqube_issues.s = 1, var_names_new_good.ratio = 0.9, reused_logic_constructor = "false",reused_logic_validation = "false", modified_lines.s = 1, education_level = "Master degree"))

own_quality_post.low <-  own_quality_post[,1]
own_quality_post.high <-  own_quality_post[,2]
own_quality_post.diff <- own_quality_post.high - own_quality_post.low

mean(own_quality_post.low)
mean(own_quality_post.high)
...
ggplot(data.frame(x = own_quality_post.diff)) + geom_histogram(aes(x=x), stat = "count") +
  labs(
    title = "Quality rating diff, high debt vs low debt",
    subtitle =,
    x ="observations",
    y = "quality impact self rating difference"
  ) +
  theme_minimal() +
