...
d.both_compleeted <- d %>% semi_join(d.sessions.completed, by = "session")
...
d.both_compleeted$work_experience_programming.s = scale(d.both_compleeted$work_experience_programming)
d.both_compleeted$work_experience_java.s = scale(d.both_compleeted$work_experience_java)
```
...
extendable_model <- function(base_name, base_formula, base_priors, family, data) {
  function(additinal_variable, additional_priors = NULL, only_priors = FALSE, sample_prior = "no") {
    
    priors <- base_priors
    if (!is.null(additional_priors)) {
      priors <- c(priors, additional_priors)
    }
    ...
    brm(
      as.formula(paste(base_formula, additinal_variable, sep = " + ")),
      family = family,
      data = as.data.frame(data),
      prior = priors,
      empty = only_priors, # If you only want to check prior definitions
      sample_prior = sample_prior,
      backend = "cmdstanr",
      chains = 4,
      cores = 4,
      file = paste("fits", paste(base_name, additinal_variable, paste("sample_priors-", sample_prior, sep = ""), sep = "."), sep = "/"),
