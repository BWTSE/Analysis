...
extendable_model <- function(base_name, base_formula, family, data, base_priors = NULL) {
  function(additional_variables = NULL, additional_priors = NULL, only_priors = FALSE, sample_prior = "no") {
    ...
    name <- base_name
    if (!is.null(additional_variables)) {
      name <- paste(base_name, additional_variables.name, sep = ".")
    }
