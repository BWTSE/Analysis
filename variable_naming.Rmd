...
d.high <- 10 - (post %>% filter(high_debt_version == "High debt version") %>% pull(estimate))
d.low <- 10 - (post %>% filter(high_debt_version == "Low debt version") %>% pull(estimate))
sum(d.high) / sum(d.low)
```
more than 5 times more likely to introduce none-descriptive variable names