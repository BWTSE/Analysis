...
data.frame(
  task_completion = factor(
    c(post.low, post.high),
    ordered = TRUE,
    levels = c(1, 2, 3, 4),
    labels = c("Not submitted", "Does not compile", "Incorrect", "Correct")
  ), 
  debt_version = c(
    rep("Low Debt", length(post.low)),
    rep("High Debt", length(post.high))
  )
) %>%
  ggplot(aes(debt_version, fill=task_completion)) +
  geom_bar() +
  labs(title = "Dropputs per signup group") +
  ylab("Number of dropouts") +
  xlab("Version")
sum(post.diff < 0)/length(post.diff)
sum(post.diff > 0)/length(post.diff)
