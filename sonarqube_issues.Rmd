...
d.both_completed %>%
  ...
  labs(
    title = "Distribution of detected sonarqube isues for the different debt levels",
    x ="Number of sonarqube issues"
  ) +
...
```{r sonarqube_issues3-sum}
summary(sonarqube_issues3)
```

#### Random effects

```{r sonarqube_issues3-raneff}
ranef(sonarqube_issues3)
```

#### Sampling plots

```{r sonarqube_issues3-plot}
plot(sonarqube_issues3, ask = FALSE)
```

#### Posterior predictive check

```{r sonarqube_issues3-pp}
pp_check(sonarqube_issues2, nsamples = 200, type = "bars") + xlim(-1, 15)
```
