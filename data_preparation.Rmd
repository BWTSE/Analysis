...
We use the utility function to set the correct types for each data column
...
Dropouts:
```{r}
ggplot(data = d.sessions, aes(task_completion)) + geom_bar()
```

Education Level:
```{r}
ggplot(data = d.sessions.completed, aes(education_level)) + geom_bar()
```
Education filed:
```{r}
ggplot(data = d.sessions.completed, aes(education_field)) + geom_bar()
```

Work domain:
```{r}
ggplot(data = d.sessions.completed, aes(work_domain)) + geom_bar()
```
Work experience:
```{r}
ggplot(data = d.sessions.completed, aes(work_experience_programming)) + geom_histogram()

ggplot(data = d.sessions.completed, aes(work_experience_java)) + geom_histogram()
```

Sonarqube issues:

```{r}
ggplot(data = d.only_submitted %>% filter(high_debt_version == "true"), aes(sonarqube_issues)) + 
  geom_histogram()

ggplot(data = d.only_submitted %>% filter(high_debt_version == "false"), aes(sonarqube_issues)) + geom_histogram()

ggplot(data = d.only_submitted, aes(x=sonarqube_issues, fill=high_debt_version)) + geom_boxplot()
```
...
plot(sonarqube_issues1)
summary(sonarqube_issues1)

loo(sonarqube_issues0, sonarqube_issues1)
```
